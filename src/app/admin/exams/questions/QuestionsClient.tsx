...
import { QuestionKind } from "@/lib/academyHub";
import { saveQuestion, saveQuestionsBulk } from "@/actions/exam";
...
export default function QuestionsClient({ initialPackages }: { initialPackages: any[] }) {
  const packages = initialPackages;
  const [mode, setMode] = useState<"SINGLE" | "BULK">("SINGLE");
  const [packageId, setPackageId] = useState(packages[0]?.id ?? "");
  const selectedPackage = packages.find((p: any) => p.id === packageId);
  const availableSections = selectedPackage ? selectedPackage.sections : [];
  ...
  const saveSingle = async () => {
    ...
    resetQuestion();
    flash("Soal tersimpan ke database dan terhubung ke paket.");
  };
  ...
  const importBulk = async () => {
    ...
    const res = await saveQuestionsBulk(parsedBulkRecords);
    if (res.error) return flash(res.error);
    flash(`${parsedBulkRecords.length} soal berhasil diimpor sekaligus ke database.`);
    setBulkText("");
  };
    ...
    <div className="flex-col gap-8">
        ...
        <label>
          ...
          <select 
            ...
            onChange={(event) => {
              ...
              const pkg = packages.find((p: any) => p.id === newPackageId);
              if (pkg && pkg.sections.length > 0) {
                setSection(pkg.sections[0].title);
              } else {
                setSection("");
              }
            }}
      ...
      {mode === "BULK" ? (
        ...
        <div className="workspace-grid">
          ...
          <aside className="card" style={{ height: "fit-content" }}>
            <h3 className="card-title">Ringkasan Soal di Paket Ini</h3>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>Total soal di paket</span>
              <strong>{selectedPackage ? selectedPackage.questionCount : 0}</strong>
            </div>
          </aside>
        </div>
      )}
