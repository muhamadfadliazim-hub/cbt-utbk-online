        ...
        {tab === 'periods' && (
            <div><h2 className="text-2xl font-bold mb-6">Manajemen Paket UTBK</h2>
                ...
                <div className="space-y-6">
                    {/* SAFE GUARD: Check Array.isArray sebelum .map */}
                        ...
                        <div key={period.id} className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                            <div className="p-5 bg-gray-50 flex items-center justify-between border-b">
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => togglePeriodActive(period.id, period.is_active)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold text-white shadow transition ${period.is_active ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700'}`}>{period.is_active ? <><EyeOff size={16}/> Sembunyikan</> : <><Eye size={16}/> Tampilkan ke Siswa</>}</button>
                    ...
                    {(!Array.isArray(periods) || periods.length === 0) && (
                        <div className="text-center p-8 text-gray-400 italic bg-white rounded border">
                            Belum ada periode ujian. Silakan buat baru.
                        </div>
                    )}
                </div>
            </div>
