...
const AdminDashboard = ({ onLogout }) => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
            ...
            {showPeriodModal && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white p-8 rounded-[2rem] w-full max-w-sm"><input className="w-full p-3 border rounded-xl mb-3" placeholder="Nama Paket" value={newPeriod.name} onChange={e=>setNewPeriod({...newPeriod, name:e.target.value})}/><select className="w-full p-3 border rounded-xl mb-3" value={newPeriod.exam_type} onChange={e=>setNewPeriod({...newPeriod, exam_type:e.target.value})}><option value="UTBK">UTBK</option><option value="CPNS">CPNS</option></select><input className="w-full p-3 border rounded-xl mb-3" placeholder="Akses Paket User (ID/ALL)" value={newPeriod.access_code} onChange={e=>setNewPeriod({...newPeriod, access_code:e.target.value})}/><button onClick={createPeriod} className="w-full p-3 bg-indigo-600 text-white rounded-xl font-bold">Buat</button></div></div>}
            {showLmsModal && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><form onSubmit={handleAddLms} className="bg-white p-8 rounded-[2rem] w-full max-w-sm"><input className="w-full p-3 border rounded-xl mb-3" placeholder="Judul" value={newLms.title} onChange={e=>setNewLms({...newLms, title:e.target.value})}/><select className="w-full p-3 border rounded-xl mb-3" value={newLms.category} onChange={e=>setNewLms({...newLms, category:e.target.value})}><option value="UTBK">UTBK</option><option value="CPNS">CPNS</option></select><input className="w-full p-3 border rounded-xl mb-3" placeholder="Sub Kategori (PU/PK/TWK)" value={newLms.subcategory} onChange={e=>setNewLms({...newLms, subcategory:e.target.value})}/><input className="w-full p-3 border rounded-xl mb-3" placeholder="Folder" value={newLms.folder_name} onChange={e=>setNewLms({...newLms, folder_name:e.target.value})}/><input className="w-full p-3 border rounded-xl mb-3" placeholder="URL" value={newLms.url} onChange={e=>setNewLms({...newLms, url:e.target.value})}/><button type="submit" className="w-full p-3 bg-indigo-600 text-white rounded-xl font-bold">Simpan</button></form></div>}
            
            {/* MANUAL EDIT & PREVIEW */}
            {showManualModal && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[100]"><div className="bg-white p-8 rounded-[2rem] w-full max-w-2xl h-[90vh] overflow-y-auto"><h3 className="text-xl font-bold mb-6">Editor Soal</h3><select className="w-full p-3 border rounded-xl mb-3" value={manualQ.type} onChange={e=>setManualQ({...manualQ, type:e.target.value})}><option value="PG">PG</option><option value="TKP">TKP</option><option value="ISIAN">ISIAN</option><option value="BOOLEAN">Tabel B/S</option><option value="PG_KOMPLEKS">Checkbox</option></select><textarea className="w-full p-3 border rounded-xl h-32 mb-3" placeholder="Soal (Support LaTeX)" value={manualQ.text} onChange={e=>setManualQ({...manualQ, text:e.target.value})}/><input className="w-full p-3 border rounded-xl mb-3" placeholder="URL Gambar" value={manualQ.media} onChange={e=>setManualQ({...manualQ, media:e.target.value})}/><div className="space-y-2">{manualQ.options.map((opt, idx) => (<div key={idx} className="flex gap-2 items-center"><input type="checkbox" checked={opt.is_correct} onChange={e=>{const n=[...manualQ.options]; n[idx].is_correct=e.target.checked; setManualQ({...manualQ, options:n})}}/><input className="flex-1 p-2 border rounded" value={opt.label} onChange={e=>{const n=[...manualQ.options]; n[idx].label=e.target.value; setManualQ({...manualQ, options:n})}}/><input className="w-16 p-2 border rounded" type="number" placeholder="Bobot" value={opt.score_weight} onChange={e=>{const n=[...manualQ.options]; n[idx].score_weight=parseInt(e.target.value); setManualQ({...manualQ, options:n})}}/></div>))}</div><div className="flex gap-2 mt-6"><button onClick={()=>setShowManualModal(false)} className="flex-1 p-3 bg-slate-100 rounded-xl font-bold">Batal</button><button onClick={saveManualQuestion} className="flex-1 p-3 bg-indigo-600 text-white rounded-xl font-bold flex justify-center gap-2"><Save size={18}/> Simpan</button></div></div></div>}
