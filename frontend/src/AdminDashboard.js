...
const AdminDashboard = ({ onLogout }) => {
  ...
  // --- API CALLS (DEFINISI FUNGSI) ---
  const fetchPeriods = () => fetch('http://127.0.0.1:8000/admin/periods').then(r=>r.json()).then(setPeriods);
  const fetchUsers = () => fetch('http://127.0.0.1:8000/admin/users').then(r=>r.json()).then(d=>{setUsers(d); setSelectedIds([])});
  
  const fetchRecap = () => {
      const url = selectedRecapPeriod 
        ? `http://127.0.0.1:8000/admin/recap?period_id=${selectedRecapPeriod}`
        : `http://127.0.0.1:8000/admin/recap`;
      fetch(url).then(r=>r.json()).then(setRecap).catch(err => console.error(err));
  };
  
  const fetchReleaseStatus = () => fetch('http://127.0.0.1:8000/config/release').then(r=>r.json()).then(d=>setIsReleased(d.is_released));

  // --- EFFECT 1: LOAD DATA SAAT GANTI TAB ---
  useEffect(() => {
  ...
  // --- EFFECT 2: RELOAD REKAP SAAT FILTER BERUBAH ---
  useEffect(() => {
  ...
  // --- ACTIONS MANAJEMEN ---
  const handleCreatePeriod = (e) => { e.preventDefault(); fetch('http://127.0.0.1:8000/admin/periods', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:newPeriodName})}).then(r=>r.json()).then(d=>{alert(d.message); setNewPeriodName(''); fetchPeriods();}); };
  const togglePeriodActive = (id, s) => fetch(`http://127.0.0.1:8000/admin/periods/${id}/toggle`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({is_active:!s})}).then(()=>fetchPeriods());
  const handleDeletePeriod = (id) => { if(window.confirm("Hapus Periode?")) fetch(`http://127.0.0.1:8000/admin/periods/${id}`, {method:'DELETE'}).then(()=>fetchPeriods()); };
  
  const handleUploadQuestion = (eid, f) => { 
      const d=new FormData(); d.append('file',f); 
      fetch(`http://127.0.0.1:8000/admin/upload-questions/${eid}`, {method:'POST', body:d})
      .then(r=>r.json()).then(d=>{ alert(d.message); fetchPeriods(); }); 
  };
  
  const handleDownloadTemplate = () => window.open('http://127.0.0.1:8000/admin/download-template', '_blank');

  const handlePreviewExam = (examId) => {
      fetch(`http://127.0.0.1:8000/admin/exams/${examId}/preview`)
  ...
  const handleBulkDelete = () => { if(selectedIds.length>0 && window.confirm("Hapus?")) fetch('http://127.0.0.1:8000/admin/users/delete-bulk', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({user_ids:selectedIds})}).then(r=>r.json()).then(d=>{alert(d.message); fetchUsers();}); };
  const handleAddUser = (e) => { e.preventDefault(); fetch('http://127.0.0.1:8000/admin/users', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(newUser)}).then(r=>{if(!r.ok) throw new Error("Gagal"); return r.json()}).then(()=>{alert("Sukses"); fetchUsers(); setNewUser({username:'',password:'',full_name:'', role:'student'})}).catch(e=>alert(e.message)); };
  const handleBulkUpload = (e) => { const f=e.target.files[0]; if(!f)return; const d=new FormData(); d.append('file',f); fetch('http://127.0.0.1:8000/admin/users/bulk',{method:'POST',body:d}).then(r=>r.json()).then(d=>{alert(d.message); fetchUsers();}) };
  
  const toggleRelease = () => { const n=!isReleased; if(window.confirm(n?"Buka Pengumuman?":"Tutup Pengumuman?")) fetch('http://127.0.0.1:8000/config/release', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({value:n?"true":"false"})}).then(r=>r.json()).then(d=>{setIsReleased(d.is_released); alert(d.message)}); };
  
  const handleDownloadExcel = () => {
      const url = selectedRecapPeriod 
        ? `http://127.0.0.1:8000/admin/recap/download?period_id=${selectedRecapPeriod}`
        : `http://127.0.0.1:8000/admin/recap/download`;
      ...
      <main className="flex-1 p-8 overflow-y-auto relative">
            ...
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-8">
                        {previewData.questions.length === 0 ? (<div className="text-center text-gray-400 py-10">Belum ada soal.</div>) : (
        ...
        {tab === 'periods' && (
            <div><h2 className="text-2xl font-bold mb-6">Manajemen Paket UTBK</h2>
                ...
                <div className="space-y-6">{periods.map(period => (<div key={period.id} className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden"><div className="p-5 bg-gray-50 flex items-center justify-between border-b"><div className="flex items-center gap-4"><button onClick={() => setExpandedPeriod(expandedPeriod === period.id ? null : period.id)} className="text-gray-500 hover:text-indigo-600">{expandedPeriod === period.id ? <ChevronUp size={24}/> : <ChevronDown size={24}/>}</button><div><h3 className="font-bold text-xl text-gray-800">{period.name}</h3><div className="flex items-center gap-2 mt-1"><span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${period.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>{period.is_active ? "PUBLIK" : "DRAFT"}</span></div></div></div><div className="flex gap-3"><button onClick={() => togglePeriodActive(period.id, period.is_active)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold text-white shadow transition ${period.is_active ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700'}`}>{period.is_active ? <><EyeOff size={16}/> Sembunyikan</> : <><Eye size={16}/> Tampilkan ke Siswa</>}</button><button onClick={() => handleDeletePeriod(period.id)} className="bg-red-50 text-red-600 p-2.5 rounded-lg hover:bg-red-100 border border-red-200"><Trash2 size={18}/></button></div></div>{expandedPeriod === period.id && (<div className="p-6 bg-white"><h4 className="font-bold text-gray-700 mb-4 border-b pb-2 flex items-center gap-2"><FileText size={18}/> Daftar Subtes UTBK</h4><div className="grid gap-3">{period.exams.map(exam => (<div key={exam.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-indigo-50/30 transition"><div className="flex-1"><div className="font-bold text-gray-900 text-lg">{exam.title}</div><div className="text-sm text-gray-500 flex gap-4 mt-1"><span className="bg-gray-100 px-2 py-0.5 rounded text-xs font-mono">Kode: {exam.code}</span><span className="flex items-center gap-1"><Clock size={14} className="inline"/> {exam.duration} Menit</span>{exam.questions && exam.questions.length > 0 ? <span className="text-green-600 font-bold flex items-center gap-1"><CheckCircle size={14}/> {exam.questions.length} Soal</span> : <span className="text-red-500 font-bold flex items-center gap-1"><XCircle size={14}/> 0 Soal</span>}</div></div><div className="flex items-center gap-2"><button onClick={() => handlePreviewExam(exam.id)} className="bg-indigo-100 text-indigo-700 px-3 py-2 rounded-lg hover:bg-indigo-200 transition font-bold text-sm flex items-center gap-1"><Search size={14}/> Lihat</button><label className="cursor-pointer bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 shadow flex items-center gap-2 transition"><Upload size={16}/> Upload<input type="file" accept=".xlsx" className="hidden" onChange={(e) => { if(e.target.files[0]) handleUploadQuestion(exam.id, e.target.files[0]); }}/></label></div></div>))}</div></div>)}</div>))}</div>
            </div>
        )}
            ...
            <div className="overflow-x-auto pb-20">
                <div className="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
                    <div className="flex-1">
                        <h2 className="text-2xl font-bold mb-1 text-gray-800">Rekapitulasi Nilai</h2>
                        
                        {/* DROPDOWN FILTER PERIODE */}
                        <div className="flex items-center gap-2 mt-2">
                            <Filter size={16} className="text-gray-500"/>
                            <span className="text-sm font-bold text-gray-600">Filter Paket:</span>
                            <select 
                                className="p-2 border border-gray-300 rounded-lg bg-white text-sm shadow-sm min-w-[250px] focus:ring-2 focus:ring-indigo-500 outline-none"
                                value={selectedRecapPeriod}
                                onChange={(e) => setSelectedRecapPeriod(e.target.value)}
                            >
