...
const AdminDashboard = ({ onLogout }) => {
    ...
    const refresh = useCallback(() => { fetch(`${API_URL}/admin/users`).then(r=>r.json()).then(setUsers); fetch(`${API_URL}/admin/periods`).then(r=>r.json()).then(setPeriods); }, []);
            ...
            <main className="flex-1 p-8 ml-64 overflow-y-auto min-h-screen">
                {analytics && (
                    <div className="mb-8 p-6 bg-white rounded-[2rem] shadow-xl border border-slate-100 relative">
                        ...
                        <h3 className="font-bold text-xl mb-4">Analisis Hasil</h3>
                        <div className="grid grid-cols-2 gap-8">
                            <div><h4 className="font-bold text-sm text-slate-500 mb-3">Top 10 Peringkat</h4>{analytics.leaderboard.map((l,i)=>(<div key={i} className="flex justify-between p-3 bg-slate-50 rounded-xl"><span className="font-bold">#{i+1} {l.name}</span><span className="font-mono text-indigo-600">{l.score}</span></div>))}</div>
                            <div className="h-64"><ResponsiveContainer><BarChart data={analytics.stats}><XAxis dataKey="no" fontSize={10}/><YAxis fontSize={10}/><Tooltip/><Bar dataKey="pct" fill="#6366f1" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></div>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'exams' && (
                    <>
                        <div className="flex justify-between mb-6"><h1 className="text-3xl font-black">Bank Soal</h1><button onClick={()=>setShowPeriodModal(true)} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold flex items-center gap-2"><Plus size={18}/> Buat Paket</button></div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {periods.map(p => (
                                <div key={p.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
                                    <div className="flex justify-between items-center mb-4">
                                        <div><h3 className="font-bold text-lg">{p.name}</h3><span className="text-xs bg-slate-100 px-2 py-1 rounded font-bold text-slate-500">{p.exam_type} {p.access_code?`[${p.access_code}]`:''} {p.show_result?'(Show Score)':'(Hide Score)'}</span></div>
                                    ...
                                    <div className="space-y-2">{p.exams.map(e => (<div key={e.id} className="flex gap-2 items-center p-3 border rounded-xl bg-slate-50"><span className="font-bold text-xs flex-1">{e.title} ({e.duration}m)</span><button onClick={()=>showStats(e.id)} className="p-2 bg-white border rounded-lg hover:border-indigo-500 text-indigo-600"><BarChart2 size={16}/></button><button onClick={()=>openPreview(e.id)} className="p-2 bg-white border rounded-lg hover:border-indigo-500 text-indigo-600"><Eye size={16}/></button><label className="p-2 bg-white border rounded-lg cursor-pointer hover:border-indigo-500 text-indigo-600"><Upload size={16}/><input type="file" className="hidden" onChange={x=>handleUploadSoal(e.id, x.target.files[0])}/></label><button onClick={()=>openManualInput(e.id)} className="p-2 bg-white border rounded-lg text-xs font-bold text-slate-600">Edit</button></div>))}</div>
                                </div>
                            ))}
                        </div>
                    </>
                ...
                {activeTab === 'users' && (
                    <>
                        <div className="flex justify-between mb-6"><h1 className="text-3xl font-black">Data Peserta</h1><div className="flex gap-2"><label className="px-4 py-2 bg-emerald-600 text-white rounded-xl font-bold cursor-pointer flex items-center gap-2"><FileSpreadsheet size={18}/> Import<input type="file" className="hidden" onChange={handleImportUser}/></label><button onClick={()=>setShowUserModal(true)} className="px-4 py-2 bg-indigo-600 text-white rounded-xl font-bold flex items-center gap-2"><Plus size={18}/> Manual</button></div></div>
                        <div className="bg-white rounded-[2rem] shadow-sm border overflow-hidden"><table className="w-full text-left text-sm"><thead className="bg-slate-50 font-bold text-slate-500"><tr><th className="p-4">Nama</th><th className="p-4">Username</th><th className="p-4">Role</th><th className="p-4">Group</th></tr></thead><tbody>{users.map(u=>(<tr key={u.id} className="border-b"><td className="p-4 font-bold">{u.full_name}</td><td className="p-4">{u.username}</td><td className="p-4 uppercase text-xs font-bold text-slate-400">{u.role}</td><td className="p-4"><span className="bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-bold text-xs">{u.group_code}</span></td></tr>))}</tbody></table></div>
                    </>
                )}
            </main>
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white p-8 rounded-[2rem] w-full max-w-sm shadow-2xl">
                        ...
                        <div className="space-y-3">
                            ...
                            <select className="w-full p-3 border rounded-xl bg-white" value={newPeriod.exam_type} onChange={e=>setNewPeriod({...newPeriod, exam_type:e.target.value})}><option value="UTBK">UTBK</option><option value="CPNS">CPNS</option><option value="TKA">TKA</option><option value="MANDIRI">MANDIRI</option></select>
                            ...
                            <div className="flex items-center gap-2"><input type="checkbox" checked={newPeriod.show_result} onChange={e=>setNewPeriod({...newPeriod, show_result:e.target.checked})}/><label className="text-sm">Tampilkan Nilai & Pembahasan</label></div>
                            <div className="flex items-center gap-2"><input type="checkbox" checked={newPeriod.can_finish_early} onChange={e=>setNewPeriod({...newPeriod, can_finish_early:e.target.checked})}/><label className="text-sm">Boleh Selesai Lebih Awal</label></div>
                        </div>
                        <div className="flex gap-2 mt-6"><button onClick={()=>setShowPeriodModal(false)} className="flex-1 p-3 bg-slate-100 rounded-xl font-bold">Batal</button><button onClick={createPeriod} className="flex-1 p-3 bg-indigo-600 text-white rounded-xl font-bold">Buat</button></div>
                    </div>
                </div>
            )}

            {showManualModal && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[100]"><div className="bg-white p-8 rounded-[2rem] w-full max-w-2xl h-[90vh] overflow-y-auto shadow-2xl"><h3 className="text-xl font-bold mb-6">Tambah Soal Manual</h3><textarea className="w-full p-4 border rounded-xl mb-4 h-32" placeholder="Teks Soal (Bisa LaTeX $...$)" value={manualQ.text} onChange={e=>setManualQ({...manualQ, text:e.target.value})}></textarea><textarea className="w-full p-4 border rounded-xl mb-4 h-24" placeholder="Pembahasan" value={manualQ.explanation} onChange={e=>setManualQ({...manualQ, explanation:e.target.value})}></textarea><div className="grid grid-cols-2 gap-4 mb-4"><input className="p-3 border rounded-xl" placeholder="URL Gambar (Opsional)" value={manualQ.media} onChange={e=>setManualQ({...manualQ, media:e.target.value})}/><input className="p-3 border rounded-xl" type="number" step="0.1" placeholder="Bobot Kesulitan (1.0)" value={manualQ.difficulty} onChange={e=>setManualQ({...manualQ, difficulty:parseFloat(e.target.value)})}/></div><div className="space-y-2"><p className="font-bold text-sm">Pilihan Jawaban (Centang Kunci / Isi Bobot TKP)</p>{manualQ.options.map((opt, idx) => (<div key={idx} className="flex gap-2 items-center"><input type="checkbox" checked={opt.is_correct} onChange={e=>{const newOpts=[...manualQ.options]; newOpts[idx].is_correct=e.target.checked; setManualQ({...manualQ, options:newOpts})}}/><span className="font-bold w-6">{opt.idx}</span><input className="flex-1 p-2 border rounded-lg" value={opt.label} onChange={e=>{const newOpts=[...manualQ.options]; newOpts[idx].label=e.target.value; setManualQ({...manualQ, options:newOpts})}} placeholder={`Opsi ${opt.idx}`}/><input className="w-16 p-2 border rounded-lg" type="number" placeholder="Bobot" value={opt.score_weight||0} onChange={e=>{const newOpts=[...manualQ.options]; newOpts[idx].score_weight=parseInt(e.target.value); setManualQ({...manualQ, options:newOpts})}}/></div>))}</div><div className="flex gap-2 mt-6"><button onClick={()=>setShowManualModal(false)} className="flex-1 p-3 bg-slate-100 rounded-xl font-bold">Batal</button><button onClick={saveManualQuestion} className="flex-1 p-3 bg-indigo-600 text-white rounded-xl font-bold">Simpan Soal</button></div></div></div>
            )}
