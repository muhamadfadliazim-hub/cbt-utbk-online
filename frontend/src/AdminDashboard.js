...
const AdminDashboard = ({ onLogout }) => {
  ...
  const handlePreviewExam = (examId) => { 
      ...
      .catch(err => alert("Preview Gagal: " + err.message)); 
  };
                        ...
                        <div className="space-y-2">
                            {users.filter(u => u.role === 'student').map(u => (
                                <label key={u.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 border rounded cursor-pointer">
                                    <input type="checkbox" className="w-5 h-5 accent-indigo-600" checked={selectedWhitelist.includes(u.username)} onChange={() => toggleUserWhitelist(u.username)} />
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border mb-8">
                    <div className="flex gap-4 items-end">
                        ...
                        <div className="w-1/3">
                            ...
                            <div onClick={() => setShowUserModal(true)} className="w-full p-2 border rounded bg-gray-50 cursor-pointer flex justify-between items-center hover:bg-gray-100">
                                <span className="text-sm text-gray-600">{selectedWhitelist.length > 0 ? `${selectedWhitelist.length} Peserta Terpilih` : (allowedUsers || "Semua Peserta (Public)")}</span>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            ...
                                            {period.allowed_usernames && <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded font-bold">TERBATAS ({period.allowed_usernames.split(',').length})</span>}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <button onClick={() => togglePeriodSubmit(period.id, period.allow_submit)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold text-white shadow transition ${period.allow_submit ? 'bg-blue-600 hover:bg-blue-700' : 'bg-red-500 hover:bg-red-600 ring-2 ring-red-300'}`} title={period.allow_submit ? "Siswa BISA Submit" : "Siswa TIDAK BISA Submit"}>{period.allow_submit ? <><Unlock size={16}/> Submit: ON</> : <><Lock size={16}/> Submit: OFF</>}</button>
                                            ...
                                            {r.completed_exams && r.completed_exams.length > 0 ? (
                                                r.completed_exams.map(exam => (
                                                    <button 
                                                        ...
                                                        title={`Klik untuk menghapus nilai ${exam.code} siswa ini`}
                                                    >
                                                        {exam.code} ×
                                                    </button>
                                                ))
                                            ) : <span className="text-gray-300 text-xs italic">Belum ada</span>}
                                        </div>
                                    </td>
                                </tr>
                            ))}
