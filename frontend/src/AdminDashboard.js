...
const AdminDashboard = ({ onLogout, apiUrl }) => {
  ...
  const filteredUsers = users.filter(u => 
    ...
    (selectedSchoolFilter === 'Semua' || u.school === selectedSchoolFilter)
  );
  ...
  // FUNGSI TAMBAH SISWA YANG SUDAH DIPERBAIKI (VALIDASI + ALERT)
  const handleAddUser = async () => {
    if (!newUser.username || !newUser.password || !newUser.full_name) {
        return alert("Mohon isi Username, Nama, dan Password!");
    }
    try {
    ...
    } catch (error) { alert("Error koneksi ke server."); }
  };

  const handleUploadUsers = async (e) => { 
      const file = e.target.files[0]; const formData = new FormData(); formData.append('file', file); 
      await fetch(`${apiUrl}/admin/users/bulk`, { method: 'POST', body: formData }); fetchData(); 
  };
  ...
  const renderRecap = () => {
    ...
    if (studentWithResults.length === 0) return <div className="p-8 text-center text-slate-400">Belum ada data nilai untuk filter ini.</div>;
    ...
    <div className="min-h-screen bg-slate-100 flex font-sans text-slate-800">
      ...
      <aside className="w-64 bg-white border-r flex flex-col fixed h-full z-10 shadow-lg">
        ...
        <nav className="flex-1 p-4 space-y-2">
            {[{id:'periods',l:'Ujian',i:Clock},{id:'users',l:'Siswa',i:Users},{id:'recap',l:'Rekap',i:BarChart3},{id:'config',l:'Pengaturan',i:Settings}].map(x=>(
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        {/* TAB UJIAN (DENGAN PREVIEW SOAL & TARGET SEKOLAH) */}
        {tab === 'periods' && (
        ...
        {(tab === 'users' || tab === 'recap') && (
            <div className="mb-6 flex gap-4 items-center bg-white p-4 rounded-xl border shadow-sm sticky top-0 z-10">
                ...
                <select value={selectedSchoolFilter} onChange={e => setSelectedSchoolFilter(e.target.value)} className="bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 font-medium outline-none focus:ring-2 focus:ring-indigo-500">
                    {schoolList.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                {tab === 'recap' && (
                    <a href={`${apiUrl}/admin/recap/download-pdf?school=${selectedSchoolFilter}`} target="_blank" rel="noreferrer" className="ml-auto bg-red-600 text-white px-4 py-2 rounded-lg font-bold text-sm shadow hover:bg-red-700 transition flex items-center gap-2"><FileText size={16}/> Download PDF ({selectedSchoolFilter})</a>
                )}
            </div>
        )}
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-white p-6 rounded-2xl shadow-sm border flex justify-between">
                ...
                <div className="flex gap-2 bg-slate-50 p-2 rounded-xl border">
                    ...
                    <input placeholder="Pass" className="bg-transparent px-2 w-24 outline-none border-l" value={newUser.password} onChange={e=>setNewUser({...newUser, password:e.target.value})}/>
                    <button onClick={handleAddUser} className="bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700 transition"><Plus size={18}/></button>
                </div>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
                ...
                <div className="max-h-[500px] overflow-y-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 sticky top-0"><tr><th className="p-4 w-10">#</th><th className="p-4">Nama</th><th className="p-4">Username</th><th className="p-4">Sekolah</th><th className="p-4">Password</th></tr></thead>
                        <tbody className="divide-y">
                            {filteredUsers.map(u=>(
