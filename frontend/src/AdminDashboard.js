...
const AdminDashboard = ({ onLogout }) => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row text-slate-900 font-sans">
            <aside className="w-full md:w-64 bg-[#0F172A] text-white p-4 md:p-6 flex flex-row md:flex-col justify-between items-center md:items-start shadow-2xl shrink-0">
                <div className="md:mb-10"><h2 className="text-xl md:text-2xl font-black italic text-indigo-400">EduPrime</h2><p className="text-[10px] text-slate-500 hidden md:block uppercase tracking-widest">Admin Panel</p></div>
                <nav className="flex md:flex-col gap-2 md:gap-4 md:flex-1">
                    <button onClick={()=>setActiveTab('users')} className={`p-2 md:p-3 rounded-xl ${activeTab==='users'?'bg-indigo-600':'hover:bg-white/5'}`}><Users size={20}/></button>
                    <button onClick={()=>setActiveTab('exams')} className={`p-2 md:p-3 rounded-xl ${activeTab==='exams'?'bg-indigo-600':'hover:bg-white/5'}`}><Database size={20}/></button>
                    <button onClick={()=>setActiveTab('lms')} className={`p-2 md:p-3 rounded-xl ${activeTab==='lms'?'bg-indigo-600':'hover:bg-white/5'}`}><BookOpen size={20}/></button>
                </nav>
                <button onClick={onLogout} className="p-2 md:p-3 bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-600 hover:text-white transition-all"><LogOut size={20}/></button>
            </aside>

            <main className="flex-1 p-4 md:p-10 overflow-y-auto">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
                    <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tight">{activeTab}</h1>
                    {activeTab === 'users' && (
                    ...
                    {activeTab === 'exams' && <button onClick={()=>setShowPeriodModal(true)} className="bg-indigo-600 text-white px-4 py-3 rounded-xl font-bold flex items-center gap-2 text-sm"><Plus/> Paket Baru</button>}
                    {activeTab === 'lms' && <button onClick={()=>setShowLmsModal(true)} className="bg-indigo-600 text-white px-4 py-3 rounded-xl font-bold flex items-center gap-2 text-sm"><Plus/> Tambah Materi</button>}
                </div>

                {activeTab === 'users' && (
                    <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 overflow-x-auto">
                        <table className="w-full text-left min-w-[600px]"><thead className="bg-slate-50 border-b text-slate-500 font-bold uppercase text-xs"><tr><th className="p-4 md:p-6 w-10 text-center"><Square size={18} onClick={toggleSelectAll} className="cursor-pointer"/></th><th className="p-4 md:p-6">Nama Lengkap</th><th className="p-4 md:p-6">Username</th><th className="p-4 md:p-6">Role</th></tr></thead><tbody>{users.map(u => (<tr key={u.id} className={`border-b hover:bg-slate-50 cursor-pointer ${selectedUsers.includes(u.id)?'bg-indigo-50':''}`} onClick={()=>toggleSelect(u.id)}><td className="p-4 md:p-6 text-center">{selectedUsers.includes(u.id)?<CheckSquare className="text-indigo-600" size={18}/>:<Square className="text-slate-300" size={18}/>}</td><td className="p-4 md:p-6 font-bold text-sm md:text-base">{u.full_name}</td><td className="p-4 md:p-6 font-mono text-xs md:text-sm">{u.username}</td><td className="p-4 md:p-6"><span className="bg-slate-100 px-3 py-1 rounded text-xs font-bold uppercase">{u.role}</span></td></tr>))}</tbody></table>
                    </div>
                )}
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        {periods.map(p => (
                            <div key={p.id} className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100">
                                <div className="flex justify-between items-center mb-4"><h3 className="text-xl md:text-2xl font-black">{p.name} <span className="text-indigo-600 text-sm md:text-lg">[{p.exam_type}]</span></h3><button onClick={()=>fetch(`${API_URL}/admin/periods/${p.id}`,{method:'DELETE'}).then(refresh)} className="text-rose-400 hover:text-rose-600"><Trash2 size={18}/></button></div>
                                <div className="space-y-3">{p.exams?.map(e => (<div key={e.id} className="p-4 bg-slate-50 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center hover:bg-slate-100 transition-all gap-3"><div><p className="font-bold text-slate-700 text-sm">{e.title}</p><p className="text-[10px] text-slate-400">Order: {e.order_index} | {e.duration} m</p></div><div className="flex gap-2 w-full md:w-auto"><button onClick={()=>openPreview(e.id)} className="p-2 bg-indigo-100 text-indigo-600 rounded-lg flex-1 md:flex-none justify-center flex"><Eye size={14}/></button><label className="p-2 bg-white border rounded-lg cursor-pointer hover:border-indigo-500 text-indigo-600 flex items-center justify-center gap-2 text-xs font-bold flex-1 md:flex-none"><Upload size={14}/> Excel<input type="file" className="hidden" onChange={x=>handleUploadSoal(e.id, x.target.files[0])}/></label></div></div>))}</div>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {activeTab === 'lms' && (
                    <div className="grid gap-6 md:grid-cols-3">
                        {materials.map(m => (
                            <div key={m.id} className="bg-white p-6 rounded-3xl shadow-lg border"><h4 className="font-bold mb-2 text-sm">{m.title}</h4><div className="flex justify-between mt-4"><button onClick={()=>window.open(m.content_url)} className="text-indigo-600 font-bold text-xs">BUKA</button><button onClick={()=>fetch(`${API_URL}/materials/${m.id}`,{method:'DELETE'}).then(refresh)} className="text-rose-400"><Trash2 size={16}/></button></div></div>
                        ))}
                    </div>
                )}
            </main>
            ...
            {previewExamId && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 md:p-6 z-[100] backdrop-blur-sm">
                    <div className="bg-white w-full max-w-4xl h-[90vh] rounded-[2rem] overflow-hidden flex flex-col relative">
                        <div className="bg-indigo-900 p-4 md:p-6 flex justify-between items-center text-white"><h3 className="font-bold text-lg md:text-xl">Preview ({previewQuestions.length} Soal)</h3><button onClick={()=>setPreviewExamId(null)} className="p-2 bg-white/10 rounded-full hover:bg-rose-500"><X size={20}/></button></div>
                        <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 bg-slate-50">{previewQuestions.map((q, i) => (<div key={q.id} className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200"><div className="font-bold text-indigo-600 mb-2 text-xs">NO {i+1} ({q.type})</div>{q.passage && <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 mb-4 text-xs md:text-sm font-serif leading-relaxed whitespace-pre-wrap"><RenderPreview text={q.passage}/></div>}{q.media && <img src={q.media} alt="Soal" className="h-32 md:h-40 object-contain mb-4 border rounded"/>}<div className="text-sm md:text-lg font-medium mb-4"><RenderPreview text={q.text}/></div><div className="grid gap-2">{q.options.map(o => (<div key={o.id} className="p-3 border rounded-xl flex gap-3 items-center text-sm"><span className="font-bold w-6 h-6 bg-slate-200 flex items-center justify-center rounded-full text-xs shrink-0">{o.id}</span><RenderPreview text={o.label}/></div>))}</div></div>))}</div>
                    </div>
                </div>
            )}
