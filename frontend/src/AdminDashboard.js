...
const AdminDashboard = ({ onLogout }) => {
    ...
    const handleBulkDelete = () => {
        if (window.confirm("PERINGATAN: Hapus SELURUH peserta? Akun admin tetap aman.")) {
            fetch(`${API_URL}/admin/users/bulk-delete`, {method:'POST'}).then(refresh);
        }
    };
        ...
        <div className="min-h-screen bg-slate-50 flex text-slate-900 overflow-hidden font-sans">
            ...
            <aside className="w-72 bg-[#0F172A] text-white p-8 flex flex-col shadow-2xl">
                ...
                <nav className="space-y-4 flex-1">
                    <button onClick={() => setActiveTab('users')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all ${activeTab==='users'?'bg-indigo-600 shadow-lg shadow-indigo-500/50':'hover:bg-white/5 text-slate-400'}`}><Users/> Peserta</button>
                    <button onClick={() => setActiveTab('exams')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all ${activeTab==='exams'?'bg-indigo-600 shadow-lg shadow-indigo-500/50':'hover:bg-white/5 text-slate-400'}`}><Database/> Bank Soal</button>
            ...
            <main className="flex-1 p-12 overflow-y-auto">
                <header className="flex justify-between items-center mb-12">
                    ...
                    <div className="flex gap-4">
                        {activeTab === 'users' && (
                            <>
                                <button onClick={handleBulkDelete} className="bg-rose-600 text-white px-6 py-4 rounded-2xl font-black shadow-xl flex items-center gap-2 hover:bg-rose-700 transition-all"><Trash2 size={20}/> HAPUS MASAL</button>
                                <label className="bg-emerald-600 text-white px-6 py-4 rounded-2xl font-black shadow-xl cursor-pointer flex items-center gap-2 hover:bg-emerald-700 transition-all"><FileSpreadsheet size={20}/> IMPOR EXCEL <input type="file" className="hidden" accept=".xlsx" onChange={(e)=>{const f=new FormData();f.append('file',e.target.files[0]);fetch(`${API_URL}/admin/users/bulk`,{method:'POST',body:f}).then(refresh);}}/></label>
                                <button onClick={() => setShowUserModal(true)} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl flex items-center gap-2 hover:bg-indigo-700 transition-all"><Plus/> TAMBAH MANUAL</button>
                            </>
                        )}
                        {activeTab === 'exams' && <button onClick={()=>setShowPeriodModal(true)} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl hover:bg-indigo-700 flex items-center gap-2 transition-all"><Plus/> PAKET BARU</button>}
                        {activeTab === 'lms' && <button onClick={()=>setShowLmsModal(true)} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl hover:bg-indigo-700 flex items-center gap-2 transition-all"><Plus/> TAMBAH MATERI</button>}
                    </div>
                </header>
                            ...
                            <tbody>
                                {users.map(u => (
                                    <tr key={u.id} className="border-b hover:bg-slate-50/50 transition-all font-bold">
                                        ...
                                        <td className="p-8 text-right"><button onClick={()=>fetch(`${API_URL}/admin/users/${u.id}`,{method:'DELETE'}).then(refresh)} className="p-3 text-rose-400 hover:text-rose-600 transition-all"><Trash2 size={20}/></button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                                    ...
                                    {p.exams?.map(e => (
                                        <div key={e.id} className="p-6 bg-slate-50 rounded-3xl flex justify-between items-center group transition-all hover:bg-slate-100">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={()=>setShowManualSoal(e.id)} className="p-3 bg-indigo-600 text-white rounded-xl shadow-lg hover:bg-indigo-700 transition-all"><Edit3 size={18}/></button>
                                                <button className="p-3 bg-slate-200 text-slate-600 rounded-xl hover:bg-slate-300 transition-all"><Eye size={18}/></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        {materials.map(m => (
                            <div key={m.id} className="bg-white p-8 rounded-[3rem] shadow-xl border relative">
                                ...
                                <div className="flex justify-between items-center pt-6 border-t">
                                    <button onClick={()=>window.open(m.content_url)} className="text-indigo-600 font-black text-xs hover:underline">BUKA MATERI</button>
                                    <button onClick={()=>fetch(`${API_URL}/materials/${m.id}`,{method:'DELETE'}).then(refresh)} className="text-rose-400 hover:text-rose-600"><Trash2 size={16}/></button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {/* Modal Manual Soal Editor (Mendukung Rumus Matematika) */}
            {showManualSoal && (
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[200] flex items-center justify-center p-6">
                    <div className="bg-white rounded-[5rem] p-16 w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl relative border-4 border-indigo-500/20">
                        ...
                        <div className="space-y-10">
                            <textarea className="w-full p-10 bg-slate-50 border-4 border-slate-100 rounded-[3rem] h-64 outline-none focus:border-indigo-500 font-black text-2xl shadow-inner" placeholder="Tulis soal di sini... Gunakan $...$ untuk Matematika" value={soalData.text} onChange={e=>setSoalData({...soalData, text:e.target.value})}/>
                ...
                <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[100] flex items-center justify-center p-6">
                    <form onSubmit={handleAddUser} className="bg-white p-12 rounded-[4rem] w-full max-w-md shadow-2xl">
                        ...
                        <div className="flex gap-4 mt-10">
                            ...
                            <button type="submit" className="flex-1 py-5 bg-indigo-600 text-white font-black rounded-2xl uppercase text-xs shadow-xl">Simpan</button>
                        </div>
                    </form>
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[100] flex items-center justify-center p-6">
                    <form onSubmit={handleAddPeriod} className="bg-white p-12 rounded-[4rem] w-full max-w-md shadow-2xl">
                        ...
                        <div className="flex gap-4 mt-10">
                            ...
                            <button type="submit" className="flex-1 py-5 bg-indigo-600 text-white font-black rounded-2xl uppercase text-xs shadow-xl">Buat Paket</button>
                        </div>
                    </form>
                </div>
            )}
            ...
            {showLmsModal && (
                <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[100] flex items-center justify-center p-6">
                    <form onSubmit={handleAddLms} className="bg-white p-12 rounded-[4rem] w-full max-w-md shadow-2xl">
                        ...
                        <div className="flex gap-4 mt-10">
                            <button type="button" onClick={()=>setShowLmsModal(false)} className="flex-1 py-5 bg-slate-100 font-black rounded-2xl uppercase text-xs">Batal</button>
                            <button type="submit" className="flex-1 py-5 bg-indigo-600 text-white font-black rounded-2xl uppercase text-xs shadow-xl">Simpan</button>
                        </div>
                    </form>
                </div>
