...
const AdminDashboard = ({ onLogout }) => {
  ...
  // --- HELPER: Render Matematika (Safe Mode) ---
  ...
  const toggleConfig = (key, currentVal) => {
      ...
      fetch(`${API_URL}/config/${key}`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({value:newVal?"true":"false"})})
  ...
  const handleChangePassword = (uid) => {
      ...
      if(newPass) fetch(`${API_URL}/admin/users/${uid}/password`, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({new_password:newPass})}).then(r=>r.json()).then(d=>alert(d.message));
  };
    ...
    <div className="min-h-screen bg-gray-100 flex font-sans text-gray-800">
      <aside className="w-64 bg-indigo-900 text-white p-6 flex flex-col">
          ...
          <div className="mt-auto pt-6 border-t border-indigo-700 space-y-3">
              ...
              <button onClick={()=>toggleConfig('enable_major_selection', isMajorSelectionEnabled)} className={`w-full flex items-center justify-between px-3 py-2 rounded text-xs font-bold transition ${isMajorSelectionEnabled ? 'bg-green-600' : 'bg-red-500'}`}>Pilih Jurusan: {isMajorSelectionEnabled ? "ON" : "OFF"} {isMajorSelectionEnabled ? <Unlock size={14}/> : <Lock size={14}/>}</button>
