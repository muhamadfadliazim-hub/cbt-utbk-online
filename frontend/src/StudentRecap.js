...
const StudentRecap = ({ username, onBack }) => {
  ...
  useEffect(() => {
    fetch(`http://127.0.0.1:8000/student/recap/${username}`)
      .then(res => {
        if (!res.ok) {
            if (res.status === 404) throw new Error("Belum ada data nilai.");
  ...
  // SAFE GUARD: Pastikan reports array ada
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans flex flex-col">
      <div className="max-w-4xl mx-auto w-full flex-1">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8 flex flex-col md:flex-row gap-6">
            <div className="flex items-start gap-3 md:w-1/3 border-r border-gray-100 pr-4">
                ...
                <div><h3 className="font-bold text-gray-800">Target Siswa</h3><p className="text-xs text-gray-500">{data.full_name}</p></div>
            </div>
                ...
                reports.map((report) => {
                    ...
                    return (
                        <div key={report.period_id} className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden transition-all duration-200 hover:shadow-lg">
                            {/* Header */}
                            <div className={`p-5 cursor-pointer flex justify-between items-center ${isLulus ? 'bg-gradient-to-r from-emerald-50 to-white' : 'bg-white'}`} onClick={() => setExpandedReport(isOpen ? null : report.period_id)}>
                                <div className="flex items-center gap-4">
                                    <div className={`p-3 rounded-full flex-shrink-0 ${isLulus ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}>{isLulus ? <Trophy size={24}/> : <Activity size={24}/>}</div>
                                    <div>
                                        <h3 className="font-bold text-lg text-gray-900">{report.period_name}</h3>
                                        <div className="flex items-center gap-3 mt-1 text-sm"><span className="text-gray-500">Skor Akhir: <strong className="text-indigo-600 text-base">{report.average}</strong></span><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${isLulus ? 'bg-green-200 text-green-800' : 'bg-red-100 text-red-600'}`}>{isLulus ? "LULUS" : "TIDAK LULUS"}</span></div>
                                    </div>
                                </div>
                                <div className="text-gray-400">{isOpen ? <ChevronUp/> : <ChevronDown/>}</div>
                            </div>
                            ...
                            {isOpen && (
                                <div className="p-6 border-t border-gray-100 bg-gray-50">
                                    {isLulus ? (
                                        <div className="mb-6 p-4 bg-green-100 border border-green-200 rounded-lg text-green-800 flex items-center gap-3 animate-pulse"><CheckCircle size={24}/><div><div className="font-bold">Selamat! Anda Memenuhi Passing Grade.</div><div className="text-sm">Diterima di: <strong>{report.accepted_major}</strong></div></div></div>
                                    ) : (
                                        <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-lg text-red-700 flex items-center gap-3"><XCircle size={24}/><div><div className="font-bold">Mohon Maaf, Nilai Belum Mencukupi.</div><div className="text-sm">Tetap semangat dan coba lagi di tryout berikutnya!</div></div></div>
                                    )}
                                    ...
                                    <div className="grid gap-2">
                                        {report.details.map(item => (
                                            <div key={item.code} className="flex justify-between items-center bg-white p-3 rounded border border-gray-200 shadow-sm"><div className="flex items-center gap-3"><span className="font-bold text-gray-600 w-12">{item.code}</span><div className="text-xs text-gray-400 flex gap-2 border-l pl-3"><span className="flex items-center gap-1 text-green-600"><CheckCircle size={10}/> {item.correct}</span><span className="flex items-center gap-1 text-red-500"><XCircle size={10}/> {item.wrong}</span></div></div><span className="font-bold text-indigo-600 text-lg">{item.score}</span></div>
                                        ))}
                                    </div>
                                </div>
                            )}
      ...
      <footer className="mt-12 text-center text-gray-400 text-xs py-4 border-t border-gray-200">Dibuat oleh Muhamad Fadli Azim</footer>
    </div>
  );
};
