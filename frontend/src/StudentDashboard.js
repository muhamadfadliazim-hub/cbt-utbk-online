...
import { 
    ...
    LayoutGrid, Award, Zap, Target, Music, User, Home, Link, ChevronRight, Play, Clock
} from 'lucide-react';
      ...
      return (
          <div className="h-screen flex flex-col bg-slate-50 font-sans overflow-hidden">
              ...
              <div className="flex-1 flex overflow-hidden relative">
                  <div className="flex-1 overflow-y-auto p-4 pb-32">
                      <div className="max-w-3xl mx-auto space-y-6">
                          {q.reading_material && (
                              <div className="bg-orange-50 p-5 rounded-2xl border-l-4 border-orange-400 text-slate-800 leading-relaxed">
                                  <div className="font-bold text-orange-600 mb-2 flex items-center gap-2 text-xs uppercase"><BookOpen size={14}/> Bacaan</div>
              ...
              <div className="h-20 bg-white border-t flex items-center justify-between px-6 z-30">
                   ...
                   <button onClick={() => setCurrentQIdx(Math.min(questions.length-1, currentQIdx + 1))} disabled={currentQIdx===questions.length-1} className="p-3 rounded-full bg-blue-600 text-white disabled:opacity-50"><ChevronRight/></button>
              </div>
          </div>
      );
  }
              ...
              <>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        {periods.map(p => (
                            <div key={p.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300">
                      ...
                      {materials.map(m=>(
                          <div key={m.id} onClick={()=>window.open(m.content_url, '_blank')} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all cursor-pointer group">
                              ...
                              <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                                  ...
                                  <Link size={16} className="text-blue-600 group-hover:translate-x-1 transition-all" />
                              </div>
                          </div>
                      ))}
              ...
              <div className="p-6 space-y-8 max-w-4xl mx-auto pb-20">
                  {reviewExamData.questions.map((q,i)=>(
                      <div key={q.id} className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-50 shadow-xl space-y-6">
                          ...
                          {q.image_url && <img src={q.image_url.startsWith('http') ? q.image_url : `${API_URL}${q.image_url}`} className="max-h-64 rounded-[2rem] border-4 border-slate-50 shadow-inner mx-auto" alt="Soal"/>}
