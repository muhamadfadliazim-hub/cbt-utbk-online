            ...
            <div className="max-w-7xl mx-auto p-6 space-y-8">
                ...
                {view === 'lms' && (
                    <div>
                        ...
                        <div className="space-y-8">{Object.keys(groupedLMS).map(sub => (<div key={sub}><h3 className="font-bold text-lg text-slate-700 mb-4 px-2 border-l-4 border-indigo-500">{sub}</h3><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{groupedLMS[sub].map(folder => (<div key={folder.id} className="bg-white p-6 rounded-[2rem] shadow-sm border"><div className="flex items-center gap-3 mb-4"><div className="p-3 bg-blue-50 text-blue-600 rounded-xl"><BookOpen size={20}/></div><h3 className="font-bold">{folder.name}</h3></div><div className="space-y-2">{folder.materials.map(m => (<a key={m.id} href={m.url} target="_blank" rel="noreferrer" className="block p-3 border rounded-xl hover:border-indigo-500 text-xs font-bold text-slate-600 flex items-center gap-2">{m.type==='video'?<Video size={14}/>:<FileText size={14}/>} {m.title}</a>))}</div></div>))}</div></div>))}</div>
                    </div>
                )}
