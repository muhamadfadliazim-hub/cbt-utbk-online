...
import { Play, BarChart2, LogOut, ChevronLeft, ChevronRight, Home, BookOpen, Award, FileText, CheckCircle, XCircle, Filter } from 'lucide-react';
...
const RenderSoal = ({ text }) => {
    ...
    return (
        <span className="text-base md:text-lg leading-loose text-slate-800 font-serif">
...
const StudentDashboard = ({ user, onLogout }) => {
    ...
    // Fitur Filter Kategori Ujian (V39)
    const [filterType, setFilterType] = useState('ALL'); // UTBK, CPNS, TKA, ALL

    // Mode Ujian / Review
    const [mode, setMode] = useState(null); // 'exam' or 'review'
    ...
    // --- MODE UJIAN (STRICT TIMER) ---
    const startExam = (eid) => {
        if(!window.confirm("Mulai ujian? Waktu tidak dapat dihentikan.")) return;
        fetch(`${API_URL}/exams/${eid}`).then(r=>r.json()).then(d=>{
            setExamData(d); setMode('exam'); setQIdx(0); setTimeLeft(d.duration * 60);
        });
    };
            ...
            <div className="h-screen flex flex-col bg-white font-sans fixed inset-0 z-50">
                    ...
                    <div className={`h-full overflow-y-auto p-6 bg-white ${q.passage ? 'w-full lg:w-1/2' : 'w-full max-w-5xl mx-auto'}`}>
                        ...
                        <div className="space-y-3">
                            {q.options?.map(o => {
                                ...
                                if(isReview) {
                                    if(o.is_correct) style = "border-emerald-500 bg-emerald-50"; // Kunci Benar
                                    else if(answers[q.id] === o.id && !o.is_correct) style = "border-rose-500 bg-rose-50"; // Jawaban Salah Siswa
                                    else style = "opacity-50 border-slate-100";
                                } else {
                                    if(answers[q.id] === o.id) style = "border-blue-600 bg-blue-50 ring-1 ring-blue-400";
                                }
                                return (
                                    <button key={o.id} onClick={()=>!isReview && setAnswers({...answers, [q.id]:o.id})} disabled={isReview} className={`w-full p-4 text-left border-2 rounded-xl flex gap-4 transition-all ${style}`}>
                        ...
                        {/* PEMBAHASAN (Hanya muncul di mode review) */}
                        {isReview && (
                            <div className="mt-8 p-6 bg-yellow-50 border border-yellow-200 rounded-xl">
                                <h4 className="font-bold text-yellow-800 mb-2 flex items-center gap-2"><BookOpen size={18}/> PEMBAHASAN:</h4>
                                <div className="text-slate-700 leading-relaxed"><RenderSoal text={q.explanation || "Tidak ada pembahasan."}/></div>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="h-16 border-t bg-slate-50 flex items-center justify-between px-8">
                    ...
                    {qIdx === examData.questions.length-1 ? 
                        (!isReview && <button onClick={submitExam} className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-bold">SELESAI</button>) : 
                        <button onClick={()=>setQIdx(Math.min(examData.questions.length-1, qIdx+1))} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold"><ChevronRight/></button>
                    }
                </div>
            </div>
        );
    ...
    if(!data) return <div className="h-screen flex items-center justify-center font-bold text-slate-400">Loading V39...</div>;
        ...
        <div className="min-h-screen bg-[#F1F5F9] font-sans pb-24">
            <div className="bg-white p-4 shadow-sm mb-6 flex justify-between items-center sticky top-0 z-30">
                ...
                <div className="flex gap-3 items-center">
                    <p className="font-bold text-sm hidden md:block">{data.user.full_name}</p>
            ...
            <div className="max-w-6xl mx-auto px-4 space-y-6">
                    ...
                    <>
                        <div className="bg-white p-6 rounded-2xl shadow-sm border flex flex-col md:flex-row justify-between gap-4">
                            ...
                            <div className="flex gap-2">
                                <select className="p-2 border rounded-lg text-sm font-bold w-full md:w-64" value={selectedMajor.m1} onChange={e=>setSelectedMajor({...selectedMajor, m1:e.target.value})}>
                        ...
                        <div className="bg-indigo-900 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden flex flex-col md:flex-row items-center justify-between">
                            <div className="mb-4 md:mb-0">
                                <h3 className="font-bold mb-1 flex items-center gap-2"><BarChart2/> Peta Kekuatan</h3>
                                <p className="text-xs text-indigo-300">Analisis performa real-time.</p>
                            </div>
                            <RadarChartMock/>
                        </div>
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2">
                            {['ALL', 'UTBK', 'CPNS', 'TKA'].map(t => (
                                <button key={t} onClick={()=>setFilterType(t)} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all ${filterType===t ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200'}`}>
                                    {t === 'ALL' ? 'SEMUA UJIAN' : t}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="space-y-6">
                            {filteredPeriods.map(p => (
                                <div key={p.id} className="bg-white p-6 rounded-2xl shadow-sm border">
                                    <h3 className="font-black text-lg mb-4">{p.name} <span className="bg-indigo-50 text-indigo-600 px-2 py-1 rounded text-xs ml-2">{p.type}</span></h3>
                                        ...
                                        {p.exams.map(e => (
                                            <div key={e.id} className={`p-4 rounded-xl border-2 transition-all ${e.status==='done'?'bg-emerald-50 border-emerald-100':'bg-white hover:border-indigo-500'}`}>
                                                ...
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    {e.status==='done' ? (
                                                        <button onClick={()=>openReview(e.id)} className="bg-emerald-600 text-white px-3 py-1 rounded-lg text-xs font-bold shadow-emerald-200 shadow-lg hover:scale-105 transition-all">LIHAT PEMBAHASAN</button>
                                                    ) : (
                                                        <button onClick={()=>startExam(e.id)} className="bg-indigo-600 text-white p-2 rounded-lg"><Play size={14}/></button>
                                                    )}
                                                </div>
                                            </div>
                                        ))}
                ...
                {view === 'lms' && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-black flex items-center gap-3"><BookOpen className="text-blue-500"/> Materi Belajar</h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {data.materials?.map(m => (
                                <div key={m.id} className="bg-white p-6 rounded-2xl shadow-sm border hover:shadow-md transition-all">
                                    ...
                                    <button onClick={()=>window.open(m.content_url)} className="w-full mt-4 py-2 border-2 border-indigo-600 text-indigo-600 rounded-lg font-bold text-xs hover:bg-indigo-50">BUKA</button>
                                </div>
                            ))}
                        </div>
                    </div>
