...
const RenderSoal = ({ text }) => { if (!text) return null; const parts = text.split(/(\$[^$]+\$)/g); return (<span className="whitespace-pre-wrap leading-relaxed font-serif text-slate-800 text-lg">{parts.map((p, i) => p.startsWith('$') ? <InlineMath key={i} math={p.slice(1,-1)}/> : <span key={i} dangerouslySetInnerHTML={{ __html: p.replace(/\[B\](.*?)\[\/B\]/g, '<b>$1</b>') }} />)}</span>); };
...
const StudentDashboard = ({ user, onLogout }) => {
    ...
    if(!data) return <div className="h-screen flex items-center justify-center font-bold text-slate-400">Loading V63...</div>;
            ...
            <div className="max-w-7xl mx-auto p-6 space-y-8">
                {view === 'home' && (
                    <>
                        ...
                        {/* FIXED: Menggunakan BarChart2 di sini */}
