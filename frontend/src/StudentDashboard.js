              ...
              <div className="flex-1 flex overflow-hidden relative">
                  <div className="flex-1 overflow-y-auto p-4 pb-32">
                      <div className="max-w-3xl mx-auto space-y-6">
                          ...
                          {q.audio_url && <div className="bg-blue-50 p-4 rounded-xl flex items-center gap-3"><div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white"><Music size={20}/></div><audio controls src={`${API_URL}${q.audio_url}`} className="w-full h-8"/></div>}
                          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                              <div className="text-lg font-medium text-slate-800 mb-6">{renderText(q.text)}</div>
                              {q.image_url && <img src={`${API_URL}${q.image_url}`} alt="Soal" className="mb-6 rounded-xl border border-slate-100 max-h-64 object-contain"/>}
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-8">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">
                    <div className="relative z-10">
                        ...
                        <div className="flex gap-3">
                            ...
                            <div className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2"><Award size={14}/> Skor: {user.pg1 || 0}</div>
                        </div>
                    </div>
                                ...
                                <div className="space-y-2">
                                    {p.exams.map(e => (
                                        <div key={e.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                            ...
                                            {e.is_done ? (
                                                <button onClick={()=>openReview(e.id)} className="px-3 py-1 bg-emerald-500 text-white text-[10px] font-bold rounded-lg">Nilai</button>
                                            ) : (
                                                <button onClick={()=>startExam(e.id)} className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center"><Play size={10} fill="currentColor"/></button>
                                            )}
                                        </div>
                                    ))}
                                </div>
          ...
          {tab === 'lms' && (
              <div className="space-y-6">
                  <h3 className="font-bold text-lg flex items-center gap-2"><BookOpen className="text-blue-600"/> Materi Belajar</h3>
                  <div className="grid gap-4 md:grid-cols-2">
                      {materials.map(m=>(
              ...
              <div className="p-4 space-y-6 max-w-3xl mx-auto">
                  ...
                  {reviewExamData.questions.map((q,i)=>(
                      <div key={q.id} className="space-y-4 pb-6 border-b">
                          ...
                          {q.image_url && <img src={`${API_URL}${q.image_url}`} className="max-h-40 rounded-lg"/>}
                          ...
                          <div className="bg-blue-50 p-4 rounded-xl text-sm text-blue-800 font-medium">💡 Pembahasan: {renderText(q.explanation)}</div>
                      </div>
                  ))}
              </div>
          </div>
