...
import { 
    ...
    LayoutGrid, Award, Zap, BarChart, Target, Music, User
} from 'lucide-react';
...
const StudentDashboard = ({ user, onLogout }) => {
  ...
  // --- TAMPILAN SAAT UJIAN (FOCUS MODE) ---
                      ...
                      <div className="max-w-5xl mx-auto space-y-8">
                          ...
                          <div className="bg-white p-6 lg:p-10 rounded-3xl shadow-xl shadow-slate-200/60 border border-white">
                              ...
                              <div className="space-y-4">
                                  ...
                                  {/* Tipe soal lain sama seperti sebelumnya... */}
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-24 lg:pb-0">
      ...
      {/* MODAL TARGET & REVIEW (Sama seperti sebelumnya, disembunyikan untuk ringkas tapi tetap ada di kode asli) */}
      {showMajorModal && (
      ...
      {reviewExamData && (
          <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-[2rem] w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden shadow-2xl">
                  <div className="p-6 border-b flex justify-between items-center bg-white">
                      <div><h3 className="font-bold text-xl text-slate-800">Hasil & Pembahasan</h3><p className="text-emerald-600 font-bold text-sm flex items-center gap-1"><Award size={14}/> Skor Kamu: {Math.round(reviewExamData.score)}</p></div>
                      <button onClick={()=>setReviewExamData(null)} className="p-2 bg-slate-50 rounded-full hover:bg-rose-50 hover:text-rose-600 transition"><X size={20}/></button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-slate-50/50">
                      {reviewExamData.questions.map((q,i)=>(
                          <div key={q.id} className="p-8 border border-slate-200 rounded-3xl bg-white shadow-sm">
                              ...
                              {q.image_url && <div className="mb-6 rounded-xl overflow-hidden border border-slate-100"><img src={`${API_URL}${q.image_url}`} alt="Soal" className="max-h-64 object-contain bg-slate-50 mx-auto"/></div>}
                              <div className="mb-8 text-lg text-slate-800 leading-relaxed font-medium">{renderText(q.text)}</div>
                              <div className="space-y-3 pl-4 border-l-4 border-slate-100">
                                  {q.options.map((o,idx)=>(
                                      <div key={idx} className={`p-4 rounded-xl flex items-center gap-4 ${o.is_correct ? 'bg-emerald-50 border-2 border-emerald-100 text-emerald-800 font-bold' : 'bg-white border border-slate-100 text-slate-500'}`}>
                                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${o.is_correct?'bg-emerald-500 text-white':'bg-slate-200'}`}>{String.fromCharCode(65+idx)}</div>
                                          {renderText(o.label)} {o.is_correct && <CheckCircle size={20} className="text-emerald-500 ml-auto"/>}
                                      </div>
                                  ))}
                              </div>
                              <div className="mt-8 p-6 bg-indigo-50/50 border border-indigo-100 rounded-2xl">
                                  ...
                                  <div className="text-sm text-slate-700 leading-relaxed">{q.explanation ? renderText(q.explanation) : "Pembahasan detail belum tersedia."}</div>
                              </div>
                          </div>
                      ))}
                  </div>
