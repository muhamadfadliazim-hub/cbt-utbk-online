...
const StudentDashboard = ({ user, onLogout }) => {
    ...
    const refresh = useCallback(() => {
        ...
        fetch(`${API_URL}/majors`).then(r=>r.json()).then(d => { if(Array.isArray(d)) setMajors(d); });
    }, [user.username]);
    ...
    const startExam = (eid, finish) => { if(!window.confirm("Mulai Ujian?")) return; fetch(`${API_URL}/exams/${eid}`).then(r=>r.json()).then(d=>{ setExamData(d); setCanFinish(finish); setMode('exam'); setQIdx(0); setTimeLeft(d.duration * 60); setAnswers({}); }); };
    ...
    const openReview = (eid) => { fetch(`${API_URL}/exams/${eid}/review?username=${user.username}`).then(r=>{if(r.ok) return r.json(); throw new Error("Pembahasan dikunci admin.");}).then(d=>{ setExamData(d); setMode('review'); setQIdx(0); }).catch(e=>alert(e.message)); };
            ...
            <div className="h-screen flex flex-col bg-slate-50 font-sans fixed inset-0 z-50">
                ...
                <div className="flex-1 flex overflow-hidden relative">
                    <div className={`absolute md:relative inset-y-0 left-0 w-64 bg-white border-r transform transition-transform z-40 ${showNav?'translate-x-0':'-translate-x-full'} md:translate-x-0 flex flex-col`}>
                        ...
                        {!isReview && <div className="p-4 border-t"><button onClick={submitExam} disabled={!canFinish && timeLeft>0} className={`w-full py-3 rounded-xl font-bold ${canFinish||timeLeft===0?'bg-indigo-600 text-white hover:bg-indigo-700':'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>{canFinish||timeLeft===0?'KUMPULKAN':'BELUM BISA SELESAI'}</button></div>}
                    </div>
                    <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[#F8FAFC]">
                        ...
                        <div className="flex-1 h-full overflow-y-auto p-6 md:p-10">{q.media && <img src={q.media} className="max-h-48 rounded mb-4 shadow-sm" alt="Soal"/><div className="mb-6"><RenderSoal text={q.text}/></div><div className="space-y-3 max-w-2xl">{q.options.map(o => { let style = "bg-white border-slate-200 text-slate-700"; if(isReview) { if(o.is_correct) style = "bg-emerald-50 border-emerald-500 text-emerald-800"; else if(answers[q.id]===o.option_index) style = "bg-rose-50 border-rose-500 text-rose-800"; } else if(answers[q.id]===o.option_index) style = "bg-indigo-50 border-indigo-500 text-indigo-800 ring-1 ring-indigo-500"; return (<button key={o.id} onClick={()=>handleAnswer(q.id, o.option_index)} disabled={isReview} className={`w-full p-4 text-left border rounded-xl flex gap-4 transition-all hover:shadow-sm ${style}`}><span className="font-bold">{o.option_index}</span><span><RenderSoal text={o.label}/></span></button>) })}</div>{isReview && <div className="mt-8 p-6 bg-amber-50 rounded-xl border border-amber-100"><h4 className="font-bold text-amber-800 mb-2">PEMBAHASAN:</h4><div className="text-slate-700"><RenderSoal text={q.explanation || "-"}/></div></div>}</div>
                    </div>
                </div>
    ...
    if(!data) return <div className="h-screen flex items-center justify-center font-bold text-slate-400">Loading V57...</div>;
    const filteredPeriods = filterType === 'ALL' ? (data.periods||[]) : (data.periods||[]).filter(p => p.type === filterType);
    
    // Group LMS by SubCategory
    const groupedLMS = {};
    if(data.lms) {
                ...
                {view === 'home' && (
                    <>
                        <div className="bg-white p-6 rounded-[2rem] shadow-sm flex flex-col md:flex-row gap-4 items-center">
                            ...
                            <div className="w-full flex gap-2"><select className="p-2 border rounded w-full text-xs" value={selectedMajor.m1} onChange={e=>setSelectedMajor({...selectedMajor, m1:e.target.value})}><option>Pilihan 1</option>{majors.map(m=><option key={m.id} value={m.id}>{m.university} - {m.program}</option>)}</select><select className="p-2 border rounded w-full text-xs" value={selectedMajor.m2} onChange={e=>setSelectedMajor({...selectedMajor, m2:e.target.value})}><option>Pilihan 2</option>{majors.map(m=><option key={m.id} value={m.id}>{m.university} - {m.program}</option>)}</select><button onClick={saveMajors} className="bg-indigo-600 text-white px-4 rounded text-xs font-bold">SIMPAN</button></div>
                        </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {groupedLMS[sub].map(folder => (
                                            <div key={folder.id} className="bg-white p-6 rounded-[2rem] shadow-sm border">
                                                ...
                                                <div className="space-y-2">{folder.materials.map(m => (<a key={m.id} href={m.url} target="_blank" rel="noreferrer" className="block p-3 border rounded-xl hover:border-indigo-500 text-xs font-bold text-slate-600 flex items-center gap-2">{m.type==='video'?<Video size={14}/>:<FileText size={14}/>} {m.title}</a>))}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
