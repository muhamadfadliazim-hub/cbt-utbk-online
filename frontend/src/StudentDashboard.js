...
const RenderSoal = ({ text }) => { if (!text) return null; const parts = text.split(/(\$[^$]+\$)/g); return (<span className="whitespace-pre-wrap leading-relaxed font-serif text-slate-800 text-lg">{parts.map((p, i) => p.startsWith('$') ? <InlineMath key={i} math={p.slice(1,-1)}/> : <span key={i}>{p}</span>)}</span>); };
...
const StudentDashboard = ({ user, onLogout }) => {
    ...
    const refresh = useCallback(() => {
        fetch(`${API_URL}/student/data?username=${user.username}`).then(r=>r.json()).then(d => { setData(d); const settings = {}; if(d.periods) d.periods.forEach(p => { settings[p.id] = {show_result: p.show_result, can_finish: p.can_finish_early}; }); setPeriodSettings(settings); });
    ...
    const startExam = (eid, pid) => { if(!window.confirm("Mulai Ujian?")) return; fetch(`${API_URL}/exams/${eid}`).then(r=>r.json()).then(d=>{ setExamData({...d, periodId: pid}); setMode('exam'); setQIdx(0); setTimeLeft(d.duration * 60); setAnswers({}); }); };
    ...
    if(!data) return <div className="h-screen flex items-center justify-center font-bold text-slate-400">Loading V54...</div>;
            ...
            <div className="max-w-7xl mx-auto p-6 space-y-8">
                {view === 'home' && (
                    <>
                        ...
                        <div className="bg-white p-6 rounded-[2rem] shadow-sm"><h3 className="font-bold mb-4 flex items-center gap-2"><BarChart2 className="text-indigo-600"/> Statistik</h3><div className="space-y-3">{data.history.slice(0,3).map((h,i)=>(<div key={i} className="flex justify-between border-b pb-2"><span className="text-sm">{h.exam}</span><span className="font-bold text-indigo-600">{h.score}</span></div>))}</div></div>
