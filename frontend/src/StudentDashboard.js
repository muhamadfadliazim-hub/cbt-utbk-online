...
const StudentDashboard = ({ user, onLogout }) => {
    ...
    const handleAnswer = (qid, val, type, optId=null) => {
        ...
        setAnswers(prev => {
            ...
            if (type === 'BOOLEAN') { // Tabel Benar/Salah
                const cur = prev[qid] || {};
                return {...prev, [qid]: {...cur, [optId]: val}}; // {optId1: "B", optId2: "S"}
            }
            return {...prev, [qid]: val};
        });
    };
    ...
    if(mode && examData) {
        ...
        const allowFinish = isReview ? false : (currentSettings.can_finish || timeLeft === 0);
    ...
    if(!data) return <div className="h-screen flex items-center justify-center font-bold text-slate-400">Loading V63.1...</div>;
