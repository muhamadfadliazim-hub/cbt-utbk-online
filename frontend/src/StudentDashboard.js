        ...
        return (
            <div className="h-screen flex flex-col bg-slate-50 font-sans fixed inset-0 z-50">
                ...
                <div className="h-16 bg-white border-t flex items-center justify-between px-6 md:hidden z-50">
                    <button onClick={()=>setQIdx(Math.max(0, qIdx-1))}><ChevronLeft/></button><span className="font-bold text-slate-600">{qIdx+1} / {examData.questions.length}</span><button onClick={()=>setQIdx(Math.min(examData.questions.length-1, qIdx+1))}><ChevronRight/></button>
                </div>
            </div>
        );
    }
            ...
            <div className="max-w-7xl mx-auto p-6 space-y-8">
                                    ...
                                    <div className="space-y-3">
                                        {p.exams.map(e => (
                                            <div key={e.id} className="p-4 border rounded-xl flex justify-between items-center hover:bg-slate-50">
                                                ...
                                                {e.status==='done' ? (
                                                    p.show_result ? 
                                                    <button onClick={()=>openReview(e.id)} className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold flex items-center gap-1"><CheckCircle size={12}/>{e.score}</button> : 
                                                    <span className="text-xs font-bold text-slate-400">Menunggu Hasil</span>
                                                ) : (
                                                    <button onClick={()=>startExam(e.id, p.id)} className="p-2 bg-indigo-600 text-white rounded-lg"><Play size={14}/></button>
                                                )}
                                            </div>
                                        ))}
                                    </div>
                ...
                {view === 'ranking' && (
                    <div className="bg-white p-8 rounded-[2rem] shadow-sm border">
                        <h2 className="text-2xl font-black mb-6 flex items-center gap-3"><Award className="text-emerald-500"/> Riwayat & Peringkat</h2>
                        <table className="w-full text-left text-sm"><thead className="bg-slate-50 font-bold text-slate-500"><tr><th className="p-4">Ujian</th><th className="p-4">Tanggal</th><th className="p-4">Skor</th></tr></thead><tbody>{data.history?.map((h, i) => (<tr key={i} className="border-b"><td className="p-4 font-bold">{h.exam}</td><td className="p-4 text-xs text-slate-500">{new Date(h.date).toLocaleDateString()}</td><td className="p-4 font-black text-emerald-600">{h.score}</td></tr>))}</tbody></table>
                    </div>
                )}
            </div>
