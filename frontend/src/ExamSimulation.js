...
const ExamSimulation = ({ examData, onSubmit, onBack }) => {
  ...
  // Ambil setting allow_submit dari data ujian (backend mengirimnya)
  const canSubmitEarly = examData.allow_submit !== false; // Default true
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
        ...
        // Submit Paksa (tanpa konfirmasi window)
        setIsSubmitting(true);
        const finalAnswers = { answers: answersRef.current, username: userData.username };
        onSubmit(finalAnswers);
        return;
      ...
      <main className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`flex-1 flex flex-col ${hasReading ? 'w-1/2' : 'w-full max-w-4xl mx-auto'}`}>
            ...
            <div className="bg-white border-t p-4 flex justify-between items-center shadow-lg z-40">
                ...
                {/* TOMBOL AKHIRI: HANYA NYALA JIKA allowed ATAU DI SOAL TERAKHIR */}
                <button 
                    disabled={(!canSubmitEarly && timeLeft > 0) || isSubmitting} 
                    onClick={handleFinalSubmit} 
                    className={`px-5 py-2.5 rounded-lg shadow-md font-bold transition flex items-center gap-2 ${(canSubmitEarly || timeLeft <= 0) ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-400 text-gray-700 cursor-not-allowed'}`}
                >
                    <Save size={18}/> 
                    {!canSubmitEarly && timeLeft > 0 ? `Terkunci (${formatTime(timeLeft)})` : isSubmitting ? 'Mengirim...' : 'AKHIRI & SUBMIT'}
                </button>
