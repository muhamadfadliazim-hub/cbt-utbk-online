...
const App = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            else { setError(data.detail || "Username atau Password Salah."); }
        } catch (err) { setError("Gagal terhubung ke server."); } finally { setLoading(false); }
    };
    ...
    // --- TAMPILAN LOGIN BARU (ELEGANT LIGHT) ---
    return (
            ...
            <div className="w-full max-w-md bg-white/80 backdrop-blur-xl border border-white/50 p-10 rounded-[2.5rem] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] relative z-10 animate-in fade-in zoom-in duration-500">
                ...
                <div className="text-center mb-10">
                    ...
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">
                        Integrated Assessment System V43
                    </p>
                </div>
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-[11px] font-black text-slate-400 uppercase ml-4 tracking-wider">Nomor Identitas</label>
                        <div className="relative group">
                            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                                <User size={20}/>
                            </div>
                            <input 
                                className="w-full bg-slate-50 text-slate-800 pl-12 pr-4 py-4 rounded-2xl border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none font-bold transition-all placeholder:text-slate-400"
                    ...
                    <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white py-4 rounded-2xl font-black text-sm tracking-widest shadow-xl shadow-indigo-200 hover:shadow-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-70 disabled:cursor-not-allowed mt-6"
                    >
                ...
                <div className="mt-8 text-center border-t border-slate-100 pt-6">
                    <p className="text-[10px] text-slate-400 font-bold">
                        &copy; 2026 EDUPRIME SYSTEM &bull; POWERED BY MUHAMAD FADLI AZIM
                    </p>
                </div>
            </div>
        </div>
