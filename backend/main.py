...
def get_materials(category: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if category and category != "ALL": q = q.filter(models.Material.category == category)
    return q.all()
...
def create_material(data: MaterialCreateSchema, db: Session = Depends(get_db)):
    ...
    db.add(m); db.commit(); return {"message": "Saved"}

@app.delete("/materials/{mid}")
def delete_material(mid: int, db: Session = Depends(get_db)):
    db.query(models.Material).filter_by(id=mid).delete(); db.commit(); return {"message": "Deleted"}
...
# FIX: ENDPOINT TOGGLE VISIBILITI
...
def add_question(eid: str, d: QuestionCreateSchema, db: Session = Depends(get_db)):
...
# FIX: ENDPOINT EDIT SOAL
@app.put("/admin/questions/{qid}")
def edit_question(qid: int, d: QuestionCreateSchema, db: Session = Depends(get_db)):
...
def preview_exam(eid: str, db: Session = Depends(get_db)):
    ...
    qs = [{"id":q.id, "type":q.type, "text":q.text, "reading_material":q.reading_material, "explanation":q.explanation, "image_url":q.image_url, "audio_url":q.audio_url, "label_true":q.label_true, "label_false":q.label_false, "options":[{"id":o.option_index, "label":o.label, "is_correct":o.is_correct} for o in q.options]} for q in exam.questions]
...
async def uq(eid:str, file: UploadFile=File(...), db:Session=Depends(get_db)):
    try:
        content = await file.read(); df = pd.read_excel(io.BytesIO(content)); df.columns = df.columns.str.strip().str.title()
        db.query(models.Question).filter_by(exam_id=eid).delete(); db.commit(); c=0
        for _, r in df.iterrows():
...
def submit_exam(eid: str, data: AnswerSchema, db: Session = Depends(get_db)):
    ...
    if db.query(models.ExamResult).filter_by(user_id=user.id, exam_id=eid).first(): return {"message": "Submitted"}
    exam = db.query(models.Exam).filter_by(id=eid).options(joinedload(models.Exam.period)).first()
    qs = db.query(models.Question).filter_by(exam_id=eid).all()
    c, earned = 0, 0.0
    for q in qs:
        if check_answer_correctness(q, data.answers.get(str(q.id))): 
            c+=1; earned += 5 if exam.period.exam_type in ["CPNS","KEDINASAN"] else q.difficulty
    
    score = earned
    if exam.period.exam_type == "UTBK":
        tw = sum(q.difficulty for q in qs)
        score = 200 + ((earned/tw)*800) if tw>0 else 200
    elif exam.period.exam_type not in ["CPNS","KEDINASAN"]:
        score = (c/len(qs)*100) if qs else 0
        
    db.add(models.ExamResult(user_id=user.id, exam_id=eid, correct_count=c, wrong_count=len(qs)-c, irt_score=score)); db.commit()
...
def login(data: LoginSchema, db: Session = Depends(get_db)):
    user = db.query(models.User).filter_by(username=data.username).first()
    if user and user.password == data.password: return {"message": "OK", "username": user.username, "name": user.full_name, "role": user.role, "choice1_id":user.choice1_id, "choice2_id":user.choice2_id}
    raise HTTPException(400, "Login Gagal")

# FIX: TOMBOL ON/OFF (Kirim value sebagai string)
@app.get("/config/{key}")
...
def set_c(k:str, d:ConfigSchema, db:Session=Depends(get_db)): 
    ...
    val = str(d.value).lower() # Force string conversion
    if c: c.value = val
...
def dl_recap(period_id: Optional[int]=None, db: Session=Depends(get_db)):
    d = get_recap(period_id, db); df = pd.DataFrame([{k:v for k,v in r.items() if k!='completed_exams'} for r in d])
