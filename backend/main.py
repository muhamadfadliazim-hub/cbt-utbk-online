...
async def bulk_user_upload(file: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        ...
        # NORMALISASI KOLOM (Biar 'User', 'username', 'USERNAME' semua kebaca)
        df.columns = [str(col).strip().lower() for col in df.columns]
        
        add=0
        for _, r in df.iterrows():
            # Cari kolom yang mirip
            u = r.get('username') or r.get('user') or r.get('nis')
            p = r.get('password') or r.get('pass') or r.get('sandi')
            n = r.get('nama') or r.get('full_name') or r.get('name')
            s = r.get('sekolah') or r.get('school') or 'Umum'
            
            if pd.isna(u) or pd.isna(p) or pd.isna(n): continue
            
            # Cek duplikat
            if db.query(User).filter_by(username=str(u).strip()).first(): continue
            
            db.add(User(username=str(u).strip(), password=str(p).strip(), full_name=str(n).strip(), role='student', school=str(s).strip()))
            add+=1
...
def sub_ex(exam_id: str, d: AnswerSchema, db: Session = Depends(get_db)):
    ...
    correct, score = calculate_irt_score(questions, d.answers) # Salah panggil fungsi di versi sblmnya, harusnya custom calc
    # RE-IMPLENTASI CALC SCORE DISINI BIAR RINGKAS
