...
# --- ENDPOINTS CONFIG (PENGUMUMAN & JURUSAN) ---
@app.get("/config/{key}")
def get_config(key: str, db: Session = Depends(get_db)):
    cfg = db.query(models.SystemConfig).filter_by(key=key).first()
    default_val = "true" if key == "enable_major_selection" else "false"
    return {"value": cfg.value if cfg else default_val}

@app.post("/config/{key}")
def set_config(key: str, data: ConfigSchema, db: Session = Depends(get_db)):
    cfg = db.query(models.SystemConfig).filter_by(key=key).first()
    if not cfg:
        cfg = models.SystemConfig(key=key, value=data.value)
        db.add(cfg)
    else:
        cfg.value = data.value
    db.commit()
    return {"message": "Config updated", "value": cfg.value}
...
def download_template_soal():
    ...
    output.seek(0)
    return StreamingResponse(output, headers={'Content-Disposition': 'attachment; filename="template_soal_utbk.xlsx"'}, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')

@app.get("/admin/periods")
def get_admin_periods(db: Session = Depends(get_db)):
    # FIX: Load questions count explicitly agar tidak 0
...
def toggle_period(period_id: int, data: TogglePeriodSchema, db: Session = Depends(get_db)):
    p = db.query(models.ExamPeriod).filter(models.ExamPeriod.id == period_id).first()
...
def delete_period(period_id: int, db: Session = Depends(get_db)):
    db.query(models.ExamPeriod).filter(models.ExamPeriod.id == period_id).delete(); db.commit()
...
async def upload_exam_manual(title: str = Form(...), duration: float = Form(...), description: str = Form(...), file: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        ...
        if 'Soal' not in df.columns: return {"message": "Gagal: Kolom 'Soal' tidak ditemukan."}
...
def get_student_periods(username: str = None, db: Session = Depends(get_db)):
    ...
    for p in periods:
        ...
        # Sort Sesuai Urutan UTBK (Nama Panjang)
