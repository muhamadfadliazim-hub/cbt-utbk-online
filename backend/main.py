...
app = FastAPI(title="EduPrime V41 Fixed")
...
def init(db: Session = Depends(get_db)):
    ...
    db.commit(); return {"status": "V41 Ready"}
...
def add_user_manual(data: dict, db: Session = Depends(get_db)):
    ...
    db.commit(); return {"msg": "User Added"}

# FIX UPLOAD PESERTA BERMASALAH
    ...
    try:
        ...
        for _, r in df.iterrows():
            if not db.query(models.User).filter_by(username=str(r['username'])).first():
                # Default role 'peserta' jika kolom role kosong/tidak ada
                ...
                count += 1
        db.commit(); return {"msg": f"Berhasil menambah {count} peserta"}
...
async def bulk_majors(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    df = pd.read_excel(io.BytesIO(await file.read()))
    for _, r in df.iterrows():
        # Pastikan kolom Excel: Universitas, Prodi, Passing_Grade
    ...
    db.commit(); return {"msg": "Passing Grade Updated"}
...
async def upload_q(eid: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    for _, r in df.iterrows():
        ...
        q = models.Question(exam_id=eid, question_type=tipe, text=str(r['Soal']), passage_text=str(r['Bacaan']) if pd.notna(r.get('Bacaan')) else None, media_url=str(r['Gambar']) if pd.notna(r.get('Gambar')) else None, explanation=str(r.get('Pembahasan', '')), difficulty=float(r.get('Kesulitan', 1.0)), correct_answer_isian=str(r['Kunci']) if tipe == 'ISIAN' else None)
        db.add(q); db.commit(); db.refresh(q)
        if 'PG' in tipe:
            for char in ['A','B','C','D','E']:
                if pd.notna(r.get(f'Opsi{char}')): db.add(models.Option(question_id=q.id, label=str(r[f'Opsi{char}']), option_index=char, is_correct=str(r.get('Kunci')).strip().upper() == char))
    db.commit(); return {"msg": "OK"}
...
def add_manual(eid: str, data: ManualQuestion, db: Session = Depends(get_db)):
    ...
    db.add(q); db.commit(); db.refresh(q)
    for opt in data.options: db.add(models.Option(question_id=q.id, label=opt['label'], option_index=opt['idx'], is_correct=opt['is_correct']))
...
def get_exam(eid: str, db: Session = Depends(get_db)):
    ...
    return {"id":e.id, "title":e.title, "duration":e.duration, "questions": [{"id":q.id, "text":q.text, "passage":q.passage_text, "media":q.media_url, "type":q.question_type, "options": [{"id":o.option_index, "label":o.label} for o in q.options]} for q in e.questions]}
...
def get_review(eid: str, username: str, db: Session = Depends(get_db)):
    ...
    return {"title": e.title, "score": res.irt_score, "questions": [{"id": q.id, "text": q.text, "passage": q.passage_text, "media": q.media_url, "explanation": q.explanation, "user_answer": user_ans.get(str(q.id)), "options": [{"id":o.option_index, "label":o.label, "is_correct":o.is_correct} for o in q.options]} for q in e.questions]}
...
@app.post("/materials")
def add_mat(title:str=Form(...), type:str=Form(...), url:str=Form(...), category:str=Form(...), db:Session=Depends(get_db)): db.add(models.Material(title=title, type=type, content_url=url, category=category)); db.commit(); return {"msg":"OK"}
