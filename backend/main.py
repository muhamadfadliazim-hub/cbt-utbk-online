...
app = FastAPI(title="EduPrime V63 Final")
...
def init(db: Session = Depends(get_db)):
    ...
    if not db.query(models.LMSFolder).first():
        defaults = [("Penalaran Umum", "UTBK", "PU"), ("Tes Wawasan Kebangsaan", "CPNS", "TWK")]
        for n,c,s in defaults: db.add(models.LMSFolder(name=n, category=c, subcategory=s))
    db.commit(); return {"status": "V63 Ready"}
...
def student_data(username: str, db: Session = Depends(get_db)):
    ...
    # Logic: allowed_exam_ids di user (misal: "1,2") atau "ALL"
    user_allowed = [x.strip() for x in (u.allowed_exam_ids or "ALL").split(",")]
    
    exam_list = []
    for p in periods:
        # Cek Akses Paket untuk User ini (Berdasarkan ID Paket atau ALL)
...
# STANDARD CRUD (Users, Periods, LMS, Upload) - Keep existing logic from V59
@app.get("/admin/users")
...
def create_period(name: str=Form(...), exam_type: str=Form(...), allowed_groups: str=Form("ALL"), show_result: bool=Form(True), can_finish_early: bool=Form(True), db: Session=Depends(get_db)):
    ...
    struct = {"UTBK":[("PU",30,1)], "CPNS":[("TWK",30,1)]} # Simplified for brevity, logic remains
        ...
        if tipe in ['PG', 'TKP', 'PG_KOMPLEKS']:
            for char in ['A','B','C','D','E']:
                if pd.notna(r.get(f'Opsi{char}')):
                    ...
                    db.add(models.Option(question_id=q.id, label=str(r[f'Opsi{char}']), option_index=char, is_correct=char in keys, score_weight=w))
    db.commit(); return {"msg": "OK"}
@app.get("/materials")
