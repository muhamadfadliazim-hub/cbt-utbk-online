...
class Exam(Base):
    ...
    code = Column(String) 
    title = Column(String)
    # PERBAIKAN: Menggunakan Float agar bisa desimal (42.5 menit)
...
app = FastAPI(title="CBT UTBK System - Full Version")
...
def startup_event():
    try:
        # Perintah ini wajib untuk membuat tabel di database kosong
        Base.metadata.create_all(bind=engine)
        print(">>> SUKSES: Tabel Database Berhasil Dibuat/Disinkronkan! <<<")
    except Exception as e:
        print(f">>> ERROR FATAL DATABASE: {str(e)} <<<")

@app.exception_handler(Exception)
async def global_exception_handler(request: Request, exc: Exception):
    err_trace = traceback.format_exc()
    print(f"SERVER ERROR: {err_trace}")
    return JSONResponse(
        status_code=500,
...
# ==========================================
# 5. ENDPOINTS: AUTH & USERS
# ==========================================

@app.get("/")
def read_root():
    return {"status": "ok", "message": "Backend CBT Aktif & Terhubung!"}

@app.post("/login")
def login(data: LoginSchema, db: Session = Depends(get_db)):
    user = db.query(User).filter_by(username=data.username).first()
    if user and user.password == data.password:
        # Ambil data jurusan pilihan jika ada
    ...
    raise HTTPException(400, "Login Gagal. Username/Password Salah.")

@app.get("/admin/schools-list")
def get_schools_list(db: Session = Depends(get_db)):
    schools = db.query(distinct(User.school)).filter(
        User.school != None, 
        User.school != ""
    ).all()
...
def del_users(d: BulkDeleteSchema, db: Session = Depends(get_db)):
    db.query(User).filter(User.id.in_(d.user_ids)).delete(synchronize_session=False)
    db.commit()
    return {"message":"OK"}
...
# ==========================================
# 6. ENDPOINTS: PERIODE & UJIAN
# ==========================================

@app.get("/admin/periods")
def get_periods(db: Session = Depends(get_db)):
    periods = db.query(ExamPeriod).order_by(ExamPeriod.id.desc()).options(
        joinedload(ExamPeriod.exams).joinedload(Exam.questions)
    ).all()
...
def create_period(d: PeriodCreateSchema, db: Session = Depends(get_db)):
    ...
    p = ExamPeriod(name=d.name, exam_type=final_type, target_schools=d.target_schools)
    db.add(p)
    db.commit()
    db.refresh(p)
...
def delete_period(pid: int, db: Session = Depends(get_db)):
    db.query(ExamPeriod).filter_by(id=pid).delete()
    db.commit()
...
# ==========================================
# 7. ENDPOINTS: JURUSAN (MAJOR SELECTION)
# ==========================================

@app.get("/majors")
def get_majors(db: Session = Depends(get_db)):
    # Endpoint ini dipakai Frontend untuk list Jurusan
    ...
    try:
        ...
        for _, r in df.iterrows():
            if pd.notna(r['universitas']):
                db.add(Major(
                    university=str(r['universitas']).strip(), 
                    name=str(r['prodi']).strip(), 
                    passing_grade=float(r['passing_grade'])
                ))
                count += 1
        db.commit()
    ...
    try:
        ...
        for _, r in df.iterrows():
            ...
            q = Question(
                ...
                reading_material=str(r.get('bacaan')), 
                explanation=str(r.get('pembahasan')), 
                image_url=str(r.get('gambar')), 
                label1=str(r.get('label1','Benar')), 
                label2=str(r.get('label2','Salah'))
            )
            db.add(q)
            db.commit()
...
def get_stats(username: str, db: Session = Depends(get_db)):
    ...
    for r in results:
        ...
        subtest_details.append({
            "id": r.exam_id, "code": code, "subject": title, 
            "correct": r.correct_count, "wrong": r.wrong_count, "score": r.irt_score
        })
    ...
    return {
        "is_released": True, "average": avg_score, "total": total_score, 
        "status": status_text, "status_color": status_color, 
        "details": subtest_details, 
        "choice1": f"{c1.university} - {c1.name}" if c1 else "-", 
        "choice2": f"{c2.university} - {c2.name}" if c2 else "-", 
        "leaderboard": leaderboard, "radar": radar_data
    }
    ...
    for p in periods:
        ...
        for e in p.exams:
            ...
            exams_data.append({
                ...
                "duration": e.duration, "is_done": is_done, "q_count": len(e.questions)
            })
        res.append({
            "id": p.id, "name": p.name, "type": p.exam_type, 
            "mode": p.target_schools or "standard", "exams": exams_data
        })
    return res
...
def submit_exam(exam_id: str, data: AnswerSchema, db: Session = Depends(get_db)):
    ...
    if db.query(ExamResult).filter_by(user_id=user.id, exam_id=exam_id).first():
    ...
    db.add(ExamResult(
        user_id=user.id, exam_id=exam_id, correct_count=correct_count, 
        wrong_count=len(questions)-correct_count, irt_score=final_score
    ))
    db.commit()
    return {"message": "Saved", "score": final_score}

# ==========================================
# 10. ENDPOINTS: CONFIG & REPORTS
# ==========================================
...
def set_conf(d: ConfigSchema, db: Session=Depends(get_db)):
    ...
    else: db.add(SystemConfig(key=k, value=d.value))
    db.commit()
    return {"message":"OK"}
...
# ==========================================
# 11. ENDPOINT PERBAIKAN (OBAT DESIMAL)
# ==========================================

@app.get("/fix-duration-decimal")
def fix_db_duration(db: Session = Depends(get_db)):
    try:
        # Perintah paksa ubah kolom database jadi desimal
        db.execute(text("ALTER TABLE exams ALTER COLUMN duration TYPE FLOAT"))
        db.commit()
        return {"message": "BERHASIL! Database sudah support desimal (42.5 menit)."}
    except Exception as e:
